...
Multiple candidates or confidence below 0.8 prompt a friendly choice before any world change. A failed interpretation preserves the strokes and uploaded reference; **Try my drawing again** reuses the captured image and bounds with your current narration. Drawing references are held in memory for the current page, not saved across reloads. Speak your story with the microphone button beside the narration box (the words appear in the box, ready to edit), and confirmed events are answered with a caption and a spoken reaction; see Voice below. Full initial-scene generation remains pending provider integration. Text narration and captions work without audio.
