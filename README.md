...
- Live story directing uses a bounded in-memory single-flight cache per API process. Identical event snapshots and text guidance share provider work, while each response keeps the caller's request ID; failures are removed so a later request can retry.
